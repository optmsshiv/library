...
<div class="page" id="page-whatsapp">
  <div class="sec-hd"><div><div class="sec-t">💬 WhatsApp Messaging</div><div class="sec-s">Auto-send fee, enrollment & reminder messages</div></div><div style="display:flex;align-items:center;gap:8px"><div class="wa-status-dot"></div><span style="font-size:11px;color:var(--wa2);font-weight:600">WhatsApp Ready</span></div></div>
...
function renderDash(){
  ...
  document.getElementById('dashStuTable').innerHTML=recent.map(x=>{
    ...
    return `<tr class="${rowClass}">
      <td><div class="si"><div class="sav" style="background:${x.color}">${x.fname[0]+x.lname[0]}</div><div><div style="font-weight:600;font-size:12.5px">${x.fname} ${x.lname}</div><div style="font-size:10px;color:var(--tx3);font-family:var(--fm)">#${x.id}</div></div></div></td>
  ...
  document.getElementById('batchGrid').innerHTML=DB.batches.map((b,i)=>{
    ...
    for(let s=1;s<=b.total;s++){
      ...
      if(stu){
        ...
        else{cls='seat-occ';ttText=`✓ ${stu.fname} — Fee Paid`;}
      }
      cells+=`<div class="seat-cell ${cls}"><div class="seat-tooltip">${ttText}</div>${sn}</div>`;
    }
